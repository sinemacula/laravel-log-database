...
```bash
git clone git@github.com:sinemacula/laravel-log-database.git
cd laravel-log-database
composer install
```
...
```text
feat: add configurable retention pruning to the log model
fix: fall back to the file channel when the database write fails
test: cover the minimum log-level threshold
...
```bash
vendor/bin/phpunit tests/Unit/DatabaseHandlerTest.php
vendor/bin/phpunit --filter testWriteRespectsMinimumLogLevel tests/Unit/DatabaseHandlerTest.php
```
